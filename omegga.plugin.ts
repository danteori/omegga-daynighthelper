...
export default class Plugin implements OmeggaPlugin<Config, Storage> {
  ...
  startTransition = async (starti: number, stopi: number, transitionTime: number) => {
    ...
    if(starti != stopi){
      this.transitionTick(starti, (stopi - starti), 0.0, transitionTime, transitionTime/100);
    }
  }
  ...
  transitionTick = async (starti: number, stopi: number, x: number, transitionTime: number, tickRate: number) => {
    ...
    if(x < transitionTime){
    intensity = (((Math.atan((x/transitionTime - 0.5)*10)*0.365)+0.5) * stopi) + starti;
  ...
  stopHelperTimeout = () => {
    if(this.helperTimeout){
      ...
      this.omegga.broadcast(`daynighthelper stopped.`);
    }
  }
  ...
  stopTransitionTimeout = () => {
    if(this.transitionTimeout){
      ...
      this.transitionTimeout = 0;
      this.omegga.broadcast(`daynighthelper transition stopped.`);
    }
  }
